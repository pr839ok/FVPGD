...
layui.define(["jquery","laytpl"], function (exports) {
            ...
            function() {
                if(options.showCheckbox){
                    ...
                    return [
                        ...
                        '{{# }else if('+options.activeParentStatus+' && d[i]["'+options.request.parentStatus+'"]) { }}',
                        '<input type="checkbox" name="eleTree-node" lay-ignore eleTree-status="2" class="layui-hide eleTree-hideen ',
                        '{{# }else if('+options.activeParentStatus+') { }}',
                        '<input type="checkbox" name="eleTree-node" lay-ignore eleTree-status="0" class="layui-hide eleTree-hideen ',
                        '{{# }else{ }}',
    ...
    Class.prototype={
        ...
        config: {
            ...
            ignoreLazyIcon: false,      // 节点还未完结加载是否允许选中
            activeParentStatus: false,   // 是否激活自定义父节点的状态
        ...
        checkboxEvent: function() {
            ...
            options.elem.on("click",checkOnClickNode,function(e) {
                ...
                // 增加20191230 判断该节点中及子孙节点是否都加载完成
                if (!options.ignoreLazyIcon && node.find(".lazy-icon").length !== 0){

                    layui.event.call(inp, MOD_NAME, 'nodeChecked('+ _self.filter +')', {
                        node: node,
                        data: _self.reInitData(node),
                        isChecked: false,
                        ignore: true
                    });
        ...
        rightClickMenu: function() {
            ...
            options.elem.on("contextmenu",".eleTree-node-content",function(e) {
                ...
                // 自定义菜单
                if (e.definedCallback && typeof e.definedCallback === "function"){
                    e.definedCallback();
                }
